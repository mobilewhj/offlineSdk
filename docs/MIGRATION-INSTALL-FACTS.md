...
**版本差异：** 已发布的 `0.2.1` 中，`isUsable()` 与安装共用互斥锁，后台下载新包时检查旧包可能等待下载结束。`0.2.2` 移除了只读检查的取锁；文件在检查过程中或返回后均可能变化，即使返回 `true` 也不保证返回瞬间目录仍存在。页面绑定后的清理时机继续由宿主控制。见 [并发修复执行记录](EXECUTION-ISUSABLE-CONCURRENCY.md)。
